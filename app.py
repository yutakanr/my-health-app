    ...
    if pw == USER_DATA[user]["pw"]:
        ...
        # --- メイン画面 ---
            ...
            with st.form("input_form"):
                ...
                with col_w: weight = st.slider("今の体重 (kg)", 40.0, 120.0, 65.0, 0.1)
                ...
                if st.form_submit_button("保存する"):
                    try:
                        # メイン保存（体重抜き）
                        new_m = pd.DataFrame([{"日付":str(date.today()), "食生活":5, "就寝時間":bed, "起床時間":wake, "寝起き":wake_s, "寝つき":sleep_q, "行動意欲":moti, "気分":mood, "体調":cond, "総合実績":total, "睡眠時間":7.0, "メモ":memo}])[m_cols]
                        ...
                        st.error("保存失敗！スプレッドシートの『共有』でサービスアカウントを『編集者』に設定してね！")
                        st.write(f"エラー詳細: {e}")
