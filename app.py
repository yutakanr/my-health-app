...
with tabs[0]:
    if user == "テト":
        ...
        with c_form:
            ...
            with st.form("cat_form"):
                ...
                if st.form_submit_button("🐾 記録を保存"):
                    new_row = {"日付": str(date.today()), "ごはんの量": food, "水分補給": water, "おしっこ回数": pee_c, "うんち回数": poo_c, "うんちの状態": poo_s, "毛玉嘔吐": vomit, "運動量": active, "ブラッシング": brush, "総合元気度": genki}
        ...
        show_data_footer(df_clean, ["日付", "ごはんの量", "水分補給", "おしっこ回数", "うんち回数", "うんちの状態", "毛玉嘔吐", "運動量", "ブラッシング", "総合元気度"], "cat")
        ...
        with st.form("h_form"):
            ...
            if st.form_submit_button("🚀 保存"):
                new_row = {"日付": str(date.today()), "起床時間": wake, "就寝時間": sleep, "睡眠時間": sl_h, "寝つき": s_q, "寝起き": s_w, "体調": cond, "総合実績": g, "行動意欲": a, "食生活": f}
                conn.update(spreadsheet=url, worksheet=t_month, data=pd.concat([raw_df, pd.DataFrame([new_row])], ignore_index=True))
                st.cache_data.clear(); st.success("保存完了"); st.rerun()
        ...
        if not df_clean.empty:
            ...
            if existing_cols:
                ...
                chart = alt.Chart(melted_df).mark_line(point=True).encode(x=alt.X('日付:N'), y=alt.Y('数値:Q', scale=alt.Scale(domain=[0, 10])), color='項目:N').properties(height=300)
                st.altair_chart(chart, use_container_width=True)
        show_data_footer(df_clean, ["日付", "起床時間", "就寝時間", "睡眠時間", "寝つき", "寝起き", "体調", "行動意欲", "食生活", "総合実績"], "hum")
    ...
    with tabs[1]:
        ...
        if not df_clean.empty:
            ...
            if not bp_df_plot.empty:
                ...
                show_data_footer(bp_df_plot, ["日付", "血圧上1", "血圧下1", "血圧上2", "血圧下2"], "bp")
    ...
    if user == "祐介" and not st.session_state.weight_auth:
        ...
        if not df_clean.empty and "体重" in df_clean.columns:
            ...
            if not w_df_plot.empty:
                ...
                show_data_footer(w_df_plot, ["日付", "体重"], "weight")
