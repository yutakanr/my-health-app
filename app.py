...
def save_entry(url, sheet_name, new_data_dict):
    existing_df = load_data(url, sheet_name)
...
with c2:
    ...
    selected_month = st.selectbox("📅 表示月を選択", month_opts, label_visibility="collapsed")
with c3:
    # 🟢 スプレッドシートへのアクセスボタンを追加
    st.link_button("📊 シートを直接開く", url, use_container_width=True)
...
# --- タブ1: 体調記録 ---
with tabs[0]:
    df_main = load_data(url, selected_month)
        ...
        with st.form("main_form", clear_on_submit=True):
            if user == "テト":
                ...
                if st.form_submit_button("🐾 記録を保存", use_container_width=True):
                    save_entry(url, selected_month, {"日付": str(date.today()), "ごはんの量": food, "水分補給": water, "うんちの状態": poo_s, "うんち回数": poo_c, "画像URL": image_to_base64(t_img), "メモ": memo})
                    st.rerun()
            else:
                ...
                memo = st.text_area("メモ")
                if st.form_submit_button("🚀 記録を保存", use_container_width=True):
                    save_entry(url, selected_month, {"日付": str(date.today()), "睡眠時間": sl_h, "体調": cond, "寝つき": s_q, "寝起き": s_w, "総合実績": g, "食生活": f, "メモ": memo})
                    st.rerun()
    ...
    if not df_main.empty:
        ...
        if v_cols:
            ...
            chart = alt.Chart(m_df).mark_line(point=True).encode(
                x='日付:N', y='value:Q', color=alt.Color('variable:N', title='項目')
...
if user == "克己":
    with tabs[1]:
        df_bp = load_data(url, selected_month)
        with st.form("bp_form"):
            b1, b2, b3 = st.columns(3)
            with b1: u1, d1 = st.number_input("血圧上1", 0, 250, 120), st.number_input("血圧下1", 0, 200, 80)
            with b2: u2, d2 = st.number_input("血圧上2", 0, 250, 120), st.number_input("血圧下2", 0, 200, 80)
            with b3: p1, p2 = st.number_input("脈拍1", 0, 200, 70), st.number_input("脈拍2", 0, 200, 70)
            if st.form_submit_button("🩸 保存"):
                save_entry(url, selected_month, {"日付": str(date.today()), "血圧上1": u1, "血圧下1": d1, "血圧上2": u2, "血圧下2": d2, "脈拍1": p1, "脈拍2": p2})
                st.rerun()
        if not df_bp.empty:
            st.subheader("📊 血圧・脈拍推移")
            bp_c = [c for c in ["血圧上1", "血圧下1", "血圧上2", "血圧下2", "脈拍1", "脈拍2"] if c in df_bp.columns]
...
with tabs[-1]:
    if user == "祐介" and not st.session_state.weight_auth:
        ...
        df_w = load_data(url, selected_month)
        with st.form("w_form"):
            w_val = st.number_input("体重(kg)", 3.0, 150.0, 60.0, step=0.1)
            if st.form_submit_button("⚖️ 保存"):
                save_entry(url, selected_month, {"日付": str(date.today()), "体重": w_val})
                st.rerun()
        if not df_w.empty and '体重' in df_w.columns:
            st.altair_chart(alt.Chart(df_w.dropna(subset=['体重'])).mark_line(point=True, color='orange').encode(
                x='日付:N', y=alt.Y('体重:Q', scale=alt.Scale(zero=False))
            ).properties(height=300), use_container_width=True)

# --- 4. 履歴表示 ---
st.divider()
all_df = load_data(url, selected_month)
if not all_df.empty:
