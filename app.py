...
try:
    ...
    if not raw_df.empty:
        # 日付列を日付型にしてから文字列に統一して重複排除
        raw_df['日付'] = pd.to_datetime(raw_df['日付']).dt.strftime('%Y-%m-%d')
        # グラフ用に「同じ日なら最後（最新）のデータ」だけを残す
        df_for_chart = raw_df.drop_duplicates(subset=['日付'], keep='last').sort_values('日付')
    else:
...
if not raw_df.empty:
    st.subheader("📋 履歴一覧")
    st.dataframe(raw_df.sort_values("日付", ascending=False), use_container_width=True)
