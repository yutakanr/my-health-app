import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
from datetime import date

# --- 1. 設定（秘密のPWはsecretsから読み込み） ---
USER_DATA = {
    "ユーザーA": {"id": "1LwTU4uf06OgRTLkP8hWoy22Wc7Zoth_cRBxsm2jjtvE", "pw": "yusuke", "weight_pw": st.secrets["passwords"]["user_a_weight"]},
    "ユーザーB": {"id": "1nKzeIhfBj97gQJWVCioAt_BfauQPr8CVBe49LPczr50", "pw": "katsumi", "weight_pw": st.secrets["passwords"]["user_b_weight"]},
    "ユーザーC": {"id": "1KXm3qm_LzScn74-x0FUUoiyFotYGFfBzfv9b_jEboE4", "pw": "noriko", "weight_pw": st.secrets["passwords"]["user_c_weight"]}
}

st.set_page_config(page_title="生活リズム・体調ログ", layout="wide")
conn = st.connection("gsheets", type=GSheetsConnection)

# UIを整える（スマホでも見やすいサイズ）
st.markdown("""
    <style>
    div[data-baseweb="select"] { font-size: 20px !important; }
    label[data-testid="stWidgetLabel"] p { font-size: 18px !important; font-weight: bold; }
    .stButton button { width: 100%; font-weight: bold; border-radius: 10px; height: 3em; background-color: #4CAF50; color: white; }
    </style>
    """, unsafe_allow_html=True)

if "view_mode" not in st.session_state: st.session_state.view_mode = "main"

st.title("🛡️ 生活リズム・体調管理")

user = st.selectbox("👤 ユーザーを選択", ["選択してください"] + list(USER_DATA.keys()))

if user != "選択してください":
    pw = st.text_input("基本パスワード", type="password")
    if pw == USER_DATA[user]["pw"]:
        # スプレッドシートの情報をセット
        sheet_id = USER_DATA[user]["id"]
        url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/edit#gid=0"
        t_sheet = date.today().strftime("%Y-%m")
        w_sheet = f"W_{t_sheet}"

        # 読み込み用関数
        def load_sheet(name, columns):
            try:
                df = conn.read(spreadsheet=url, worksheet=name, ttl=0)
                return df if df is not None and not df.empty else pd.DataFrame(columns=columns)
            except:
                return pd.DataFrame(columns=columns)

        m_cols = ["日付", "食生活", "就寝時間", "起床時間", "寝起き", "寝つき", "行動意欲", "気分", "体調", "総合実績", "睡眠時間", "メモ"]
        w_cols = ["日付", "体重"]
        
        data = load_sheet(t_sheet, m_cols)
        w_data = load_sheet(w_sheet, w_cols)

        st.divider()
        c1, c2 = st.columns(2)
        if c1.button("📝 日報入力・履歴"): st.session_state.view_mode = "main"
        if c2.button("⚖️ 体重管理画面"): st.session_state.view_mode = "weight"

        # --- 体重管理モード ---
        if st.session_state.view_mode == "weight":
            st.header("⚖️ 体重モニタリング")
            w_pw = st.text_input("体重用パスワード", type="password")
            if w_pw == USER_DATA[user]["weight_pw"]:
                st.success("認証成功")
                if not w_data.empty:
                    df_w = w_data.copy()
                    df_w["体重"] = pd.to_numeric(df_w["体重"], errors='coerce')
                    st.line_chart(df_w.set_index("日付")["体重"])
                    st.dataframe(w_data.sort_index(ascending=False), use_container_width=True)
                else: st.info("まだ体重データがないよ")
            elif w_pw: st.error("パスワードが違うよ")

        # --- メイン日報モード ---
        else:
            with st.form("input_form"):
                col_t1, col_t2, col_w = st.columns(3)
                with col_t1: bed = st.text_input("就寝時間", "22:00")
                with col_t2: wake = st.text_input("起床時間", "06:30")
                with col_w: weight_val = st.slider("体重 (kg)", 40.0, 120.0, 65.0, 0.1)

                c1, c2, c3 = st.columns(3)
                with c1: wake_s = st.slider("寝起き", 1, 10, 5); mood = st.slider("気分", 1, 10, 5)
                with c2: sleep_q = st.slider("寝つき", 1, 10, 5); cond = st.slider("体調", 1, 10, 5)
                with c3: moti = st.slider("意欲", 1, 10, 5); total = st.slider("総合実績", 1, 10, 5)
                
                memo = st.text_area("メモ")
                
                if st.form_submit_button("この内容で保存する"):
                    try:
                        # ① メインデータの更新
                        new_m = pd.DataFrame([{
                            "日付": str(date.today()), "食生活": 5, "就寝時間": bed, "起床時間": wake,
                            "寝起き": wake_s, "寝つき": sleep_q, "行動意欲": moti, "気分": mood,
                            "体調": cond, "総合実績": total, "睡眠時間": 7.0, "メモ": memo
                        }])[m_cols]
                        data_updated = pd.concat([data, new_m], ignore_index=True)
                        conn.update(spreadsheet=url, worksheet=t_sheet, data=data_updated)

                        # ② 体重データの更新
                        new_w = pd.DataFrame([{"日付": str(date.today()), "体重": weight_val}])[w_cols]
                        w_updated = pd.concat([w_data, new_w], ignore_index=True)
                        conn.update(spreadsheet=url, worksheet=w_sheet, data=w_updated)

                        st.success("保存できたよ！今日も一日お疲れ様！")
                        st.rerun()
                    except Exception as e:
                        st.error("保存に失敗しちゃった。")
                        st.write(f"エラー詳細: {e}")

            if not data.empty:
                st.divider()
                st.subheader("📊 直近の履歴")
                st.dataframe(data.sort_index(ascending=False), use_container_width=True)

    elif pw:
        st.error("パスワードが違うよ")
