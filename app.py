...
if not df_clean.empty:
    ...
    if existing_cols:
        ...
        chart = alt.Chart(melted_df).mark_line(point=True).encode(
            ...
            y=alt.Y('数値:Q', scale=alt.Scale(domain=[0, domain_max], clamp=True), axis=alt.Axis(values=[0, 2, 4, 6, 8, 10]), title='スコア'), # 軸を固定
...
with tabs[0]:
    if user == "テト":
        ...
        with c_img:
            # 画像を表示。PILを使って開く
            # 注：画像ファイル（image_12.png）がapp.pyと同じディレクトリにある前提です
            try:
                cat_image = Image.open('image_12.png')
                st.image(cat_image, caption='テトちゃん？', use_column_width=True)
            except FileNotFoundError:
                st.error("画像ファイル(image_12.png)が見つかりません。")
        ...
        # 人間用履歴表：「総合実績」を「メモ」の後ろに入れ替える
        human_cols_reordered = ["日付", "起床時間", "就寝時間", "睡眠時間", "寝つき", "寝起き", "体調", "行動意欲", "食生活", "メモ", "総合実績"]
        show_edit_delete_section(df_clean, human_cols_reordered)
...
if user == "克己":
    with tabs[1]:
        ...
        if not df_clean.empty:
            bp_only_df = df_clean.dropna(subset=["血圧上1"])
            show_edit_delete_section(bp_only_df, ["日付"] + bp_cols)
...
with tabs[weight_tab_idx]:
    ...
    if user == "祐介" and not st.session_state.weight_auth:
        ...
        if not df_clean.empty:
            w_only_df = df_clean.dropna(subset=["体重"])
            show_edit_delete_section(w_only_df, ["日付", "体重"])
